//! `lore mcp` (§12, D-037/D-079): a Model Context Protocol server over the
...
//! query/report surfaces (`lore_ask`, `lore_show`, `lore_lint`,
//! `lore_history`), each returning the same JSON its `--json` CLI form does,
//! and never mutates the project. Hand-rolled over serde_json — no MCP-SDK
//! dependency (the wire format is a small JSON-RPC loop Lore owns outright).
...
/// `lore_ask` with `affects(...)` (the T9 eval).
fn tools_list() -> Value {
    json!({
        "tools": [
            {
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "query": {"type": "string", "description": "A Lore query, e.g. affects(Payment.ledger) or path(A, B)."},
...
fn tools_call(manifest_path: &Path, id: Value, params: &Value) -> Value {
    ...
    let result = match params.get("name").and_then(Value::as_str) {
        Some("lore_ask") => run_ask(manifest_path, &args),
        Some("lore_show") => run_show(manifest_path, &args),
        Some("lore_lint") => run_lint(manifest_path),
        Some("lore_history") => run_history(manifest_path, &args),
...
/// `lore_ask`: the §10.4 JSON for an arbitrary query, with optional `all` /
/// `max_len` mirroring `path --all --max-len N`.
fn run_ask(manifest_path: &Path, args: &Value) -> Result<Value, String> {
    let query_text = args
        .get("query")
        .and_then(Value::as_str)
        .ok_or("lore_ask requires a string \"query\" argument")?;
...
/// `lore_show`: the §10.2 card, implemented as the `show(<qname>)` query so the
/// card shape is byte-identical to `lore ask`.
fn run_show(manifest_path: &Path, args: &Value) -> Result<Value, String> {
    let qname = args
        .get("qname")
        .and_then(Value::as_str)
        .ok_or("lore_show requires a string \"qname\" argument")?;
    ask_query(manifest_path, &format!("show({qname})"), false, None)
}

/// Shared core of `lore_ask` and `lore_show`: build the graph, parse, execute,
...
/// `lore_lint`: the §12 lint JSON, full reconciliation with staleness on
/// (no_stale = false, D-079b). Identical policy/override processing as
/// `lore lint --json`, via the shared `lint::compute`.
...
/// `lore_history`: the §9.3 history JSON. The D-059 failure modes (unknown
...
fn run_history(manifest_path: &Path, args: &Value) -> Result<Value, String> {
    let qname = args
        .get("qname")
        .and_then(Value::as_str)
        .ok_or("lore_history requires a string \"qname\" argument")?;
    let p = load_project(manifest_path)?;
...
fn load_project(manifest_path: &Path) -> Result<project::Project, String> {
    project::load(manifest_path).map_err(|_| {
        format!(
            "no usable lore.toml at {} (see stderr); run lore init",
