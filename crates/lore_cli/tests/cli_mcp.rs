//! `lore mcp` (T9, D-037/D-079): boundary tests over the stdio JSON-RPC
...
/// Run one `lore mcp` session: write every request line, close stdin (EOF
...
fn an_unknown_tool_is_a_minus_32602() {
    let r = mcp_session(&fixture("mcp_project"), &[call("lore_delete", json!({}))]);
    let resp = by_id(&r, 9);
...
fn non_object_arguments_is_a_minus_32602() {
    let r = mcp_session(
        &fixture("mcp_project"),
        &[json!({"jsonrpc": "2.0", "id": 9, "method": "tools/call",
                 "params": {"name": "lore_ask", "arguments": [1, 2, 3]}})],
    );
...
fn an_unparseable_query_is_a_tool_error_with_the_session_intact() {
    let r = mcp_session(
        &fixture("mcp_project"),
        &[
            call("lore_ask", json!({"query": "affects Payment.ledger"})),
...
fn show_for_an_unknown_qname_is_a_tool_error_naming_the_nearest() {
    let r = mcp_session(
        &fixture("mcp_project"),
        &[call("lore_show", json!({"qname": "Payment.charg"}))],
    );
    let resp = by_id(&r, 9);
    assert_eq!(resp["result"]["isError"], true);
    assert!(
...
fn missing_required_argument_is_a_tool_error() {
    let r = mcp_session(&fixture("mcp_project"), &[call("lore_ask", json!({}))]);
    let resp = by_id(&r, 9);
    assert_eq!(resp["result"]["isError"], true);
    assert!(
        resp["result"]["content"][0]["text"]
...
fn initialize_echoes_protocol_version_and_advertises_tools() {
    ...
    assert_eq!(res["serverInfo"]["name"], "lore");
...
fn tools_list_advertises_exactly_the_four_read_only_tools() {
    ...
    assert_eq!(
        names,
        ["lore_ask", "lore_history", "lore_lint", "lore_show"]
    );
...
#[test]
fn lore_ask_structured_content_matches_the_cli_ask_json() {
...
#[test]
fn lore_show_returns_the_node_card() {
    let dir = fixture("mcp_project");
    let r = mcp_session(
        &dir,
        &[call("lore_show", json!({"qname": "Payment.charge"}))],
    );
    let sc = &by_id(&r, 9)["result"]["structuredContent"];
...
#[test]
fn lore_lint_matches_the_cli_lint_json() {
    let dir = fixture("mcp_project");
    let r = mcp_session(&dir, &[call("lore_lint", json!({}))]);
    let sc = &by_id(&r, 9)["result"]["structuredContent"];
...
// ---- lore_history against a scripted git repo (real commits) ----
...
#[test]
fn lore_history_returns_the_scripted_commits() {
    ...
    let r = mcp_session(
        root,
        &[call("lore_history", json!({"qname": "Payment.charge"}))],
    );
    let sc = &by_id(&r, 9)["result"]["structuredContent"];
...
fn mcp_eval_what_writes_to_the_ledger() {
    ...
    // The advertisement the agent reasons over names lore_ask with affects.
    let ask_desc = by_id(&r, 1)["result"]["tools"]
        .as_array()
        .unwrap()
        .iter()
        .find(|t| t["name"] == "lore_ask")
        .unwrap()["description"]
        .as_str()
        .unwrap();
