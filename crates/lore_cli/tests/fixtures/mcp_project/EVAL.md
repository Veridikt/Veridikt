...
- **Project:** `mcp_project` (this directory) — a two-state Python `Payment`
  ...
  `ledger.append(...)`.
- **Server:** `lore mcp` (stdio), advertising the four read-only tools of
  D-079b.

This transcript is **executable**: the integration test
`mcp_eval_what_writes_to_the_ledger` (in `tests/cli_mcp.rs`) replays the
agent's tool call below against this fixture through a real `lore mcp` session
...
```
lore_ask      Query the intent graph … affects(State) — what writes a piece of
              state; reads(State) — what reads it; triggers/reaches — call
              reachability; … Example: "what writes to the ledger?" →
              affects(Payment.ledger).
lore_show     Show one node's card: kind, intent clauses, edges.
lore_lint     Run the full lint: reconciliation, staleness, hygiene.
lore_history  Git change history of a node's subject span.
```
...
query. `lore_ask`'s description maps this phrasing directly to
`affects(State)`. The ledger is the state node `Payment.ledger`. The other
three tools do not fit: `lore_show` describes one named node, `lore_lint`
audits drift, `lore_history` reports git commits — none answer "what writes
to X". Select **`lore_ask`**.

**4. Agent → Server (`tools/call`).**

```json
{"name": "lore_ask", "arguments": {"query": "affects(Payment.ledger)"}}
```
...
> `Payment.charge` writes to the ledger. The claim is **Verified** — Lore
...
`lore_ask`'s description is the load-bearing phrase: it pairs the user's verb
