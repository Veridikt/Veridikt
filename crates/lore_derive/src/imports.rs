...
/// file set (normalized path -> index), and language-manifest texts keyed by
/// the manifest's directory (for `manifest_prefix`).
...
fn try_strategy(
    ...
    match strategy {
        ...
        // The named-impl escape hatch (D-071b). `rust_use_paths` lands with the
        // Rust derive tier (T8 step 6); no derive-tier pack selects a custom
        // strategy before then, so this arm is currently unreached.
        ImportStrategy::Custom { .. } => None,
    }
}
