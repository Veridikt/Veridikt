...
- **Line comments only.** A block is a run of contiguous line comments whose first content line is exactly `@lore`. Block comments (`/* */`, `""" """`) are **not** scanned in v1 - one rule, every language (§7.1). Codebases that prefer block-doc conventions must use line comments for intent.
...
Crate: `lore_annotations` (scanner + binder), consuming the clause grammar from `lore_intent`. Input: a source tree. Output: blocks, their subjects, and qnames - handed as data to `lore_graph`.

**Scanning (§7.1, §7.2).** The scanner finds each `@lore` block, strips each line's comment token plus at most one following space, and reassembles the remainder into one intent-block text. A block grammar of `marker_line` (`@lore`) followed by binding lines (`kind:`, `name:`) and clause lines is parsed; default `kind` is `function`. An unknown clause is `E0202` (with the nearest valid clause suggested); a clause whose value violates its grammar is `E0207` and contributes nothing to the `Intent`.
...
**Module scoping (§7.5, D-015).** A subject's qname is `<module>.<name>`, where the module comes from a `lore.toml [modules]` glob (first match wins) or a top-of-file scoping block. Steps additionally require an enclosing `kind: workflow` block in order (`E0105`).

**The `Intent` contract (§13).** Parsing yields the shared `Intent` struct: singular prose clauses (`purpose`, `owner`) as `Option`, repeatable ones (`because`, `unknown`, `assumes`) as `Vec`, and reference clauses (`affects`, `reads`, `triggers`, `emits`, `on`, `depends_on`) as `Vec<Spanned<Ref>>` - refs unresolved at this stage (resolution happens in `lore_graph`, §6.3). This struct is consumed verbatim by both phases (G-3).
...
**Before** - a plain comment that drifts silently and connects to nothing Lore can check:
...
```sh
lore ask 'show(Payment.charge)'   # qname, kind, origin, every clause verbatim, edges, statuses
lore ask 'unknown'                # every declared open question in the project
lore ask 'owner("payments-team")' # nodes by declared owner
```
...
Add a block above a declaration - comment token of the host language, first content line exactly `@lore`:
...
```sh
lore scan      # lists the block, its subject, qname, kind
lore lint      # resolution + applicability findings; E0306 if Payment.ledger is unknown
```
